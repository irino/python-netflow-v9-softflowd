...
@cython.cfunc
def human_size(size_bytes: cython.int):
    # Calculate a human readable size of the flow
    if size_bytes < 1024:
        return "%dB" % size_bytes
    size_bytes_local: cython.float = size_bytes / 1024
    units: python_string[3] = ["K", "M", "G"]
    for unit in units:
        if size_bytes_local < 1024:
            return "%.2f%s" % (size_bytes_local, unit)
        size_bytes_local = size_bytes_local / 1024


@cython.cfunc
def human_duration(seconds: cython.int):
    # Calculate human readable duration times
    if seconds < 60:
        # seconds
        return "%d sec" % seconds
    seconds_mod: cython.int = seconds % 60
    minutes: cython.int = seconds // 60
    if minutes < 60:
        # minutes
        return "%02d:%02d min" % (minutes, seconds_mod)
    # hours
    return "%d:%02d.%02d hours" % (minutes // 60, minutes % 60, seconds_mod)
